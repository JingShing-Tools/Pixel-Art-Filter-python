...
dithered_edges = dithered_edges.astype(np.uint8)  # 將掩模轉換為與圖像相同的深度
for i in range(dithered_edges.shape[0]):
    for j in range(dithered_edges.shape[1]):
        if dithered_edges[i, j] > threshold[i % 2, j % 2]:
